...
def parse_operations(path: str) -> List[Tuple[str, List[int], str]]:
    operations = []
    if not path:
        return operations
...
def build_frames(initial_values: List[int], operations: List[Tuple[str, List[int], str]], interval: float) -> List[OperationFrame]:
    ...
    for command, args, label in operations:
        ...
        if command == "append":
            ...
            if index <= 0:
                ...
                insert_index = index
            linked_list.insert(value, insert_index)
            new_node = NodeState(next_id, value)
            next_id += 1
            nodes.insert(insert_index, new_node)
            frames.append(OperationFrame(
...
def main():
    ...
    frames = build_frames(values, operations, args.interval)
    ...
    while running:
        ...
        visuals = layout_nodes(nodes_render, args.width, args.height)
        screen.fill(DEFAULT_BG_COLOR)

        panel_rect = pygame.Rect(20, 20, PANEL_WIDTH - 40, args.height - 40)
        ...
        for index in range(len(visuals) - 1):
            ...
            if current.row != next_visual.row:
                ...
                if frame.op_type == "add" and frame.added_id in {current.node_id, next_visual.node_id}:
                    link_progress = clamp(op_elapsed / max(args.arrow_interval, 0.01), 0.0, 1.0)
